...
public class SimpleOomage implements Oomage {
    ...
    @Override
    //doing a cast without verifying that the Object is a SimpleOomage wont’ work,
    // because you don’t want your code to crash if someone calls .equals with an argument that is not a SimpleOomage
    ...
    //if we override equals, we need to override hashCode as well
    @Override
    public int hashCode() {
        if (!USE_PERFECT_HASH) {
            return red + green + blue;//will cause collision
        } else {
            int hash = 0;
            hash = hash + (red / 5) * 53 * 53;
            hash = hash + (green / 5) * 53;
            hash = hash + (blue / 5);
            return hash;
        }
    }
